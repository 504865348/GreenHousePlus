...
session.setAttribute("ghId", ghId);
	...
	public String ghadmin_gh_detail(Model model,
		...
		//新增加 @RequestParam(value="ghid") String ghid
		User u = userDao.getCurrentUser(request);
		/*Greenhouse gh = greenhouseDao.findByUserId(u.getUser_id(),request);*/
		Greenhouse gh = greenhouseDao.findByghid(Integer.parseInt(ghid)); //修改过
		...
		model.addAttribute("pager", cropDao.listByPage(Cnd.where("deleted", "=", "N").and("ghid", "=", ghid), 20, 1));//新增
	...
	public String ghadmin_crop_add(Model model,
			@RequestParam(value="ghid") String ghid){
		System.out.println("温度id为"+ghid);
