...
<body>
	<div class="w11 center-block">
		...
		<c:if test="${empty pager.list}">
			<h3>系统中还没有优化控制模型库</h3>
			<a href="<%=request.getContextPath()%>/modellib/ctrlcost/add">添加模型库</a>
		</c:if>
		<c:if test="${not empty pager.list}">
			<h3 class="h3">全部调控成本模型</h3>
	    ...
	    jump: function(e, first){ 
			...
			<div style="position: absolute; right: 20px; width: 100px;">
				<input type="button" value="添加模型" class="btn bcm tcw"
					onclick="location='<%=request.getContextPath()%>/modellib/ctrlcost/add'">
			</div>
			<!-- 分页end -->
		</c:if>
	</div>
