...
<body>
	<div class="w11 center-block">
		...
		<c:if test="${empty pager.list}">
			<h3>系统中还没有调控成本模型库</h3>
			<a href="<%=request.getContextPath()%>/modellib/optctrl/add">添加模型库</a>
		</c:if>
		<c:if test="${not empty pager.list}">
			<h3 class="h3">全部优化控制模型</h3>
	...
	laypage({
	    ...
	    jump: function(e, first){ 
			...
			<div style="position: absolute; right: 20px; width: 100px;">
				<input type="button" value="添加模型" class="btn bcm"
					onclick="location='<%=request.getContextPath()%>/modellib/optctrl/add'">
			</div>
			<!-- 分页end -->
		</c:if>
	</div>
