<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<link rel="stylesheet"
	href="<%=request.getContextPath() %>/assets/css/style.css"
	type="text/css"></link>
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath() %>/assets/css/theme.css">
<link rel="stylesheet" type="text/css"
	href="<%=request.getContextPath() %>/assets/css/font-awesome.css">
<link rel="stylesheet"
	href="<%=request.getContextPath() %>/assets/css/admin.css"
	type="text/css"></link>
<script
	src="<%=request.getContextPath() %>/assets/js/jquery-2.1.1.min.js"></script>
<script src="<%=request.getContextPath() %>/assets/js/bootstrap.min.js"></script>
<%String ghid = request.getParameter("ghid");
System.out.print("ghidghidghid"+ghid);
%>

<title>添加作物信息</title>
</head>
<body>
	<form action="<%=request.getContextPath()%>/ghmgr/ghadmin/crop/add?ghid=<%=ghid %>"
		method="post" success="操作成功" fail="操作失败" forward="list?ghid=<%=ghid %>" form-ajax>
		<h3 class="h3">添加作物信息</h3>
		<div style="width: 60%; margin: 20px auto;">
			<table class="table table-bordered table-condensed">
				<tr>
					<td class="text-center">作物名称：</td>
					<td><input type="text" name="Crop_name"
						class="required form-control" /></td>
				</tr>
				<tr>
					<td class="text-center">作物编码：</td>
					<td><input type="text" name="Crop_code"
						class="required number form-control" /></td>
				</tr>
				<tr>
					<td class="text-center">作物品种：</td>
					<td><input type="text" name="Crop_type"
						class="required form-control" /></td>
				</tr>
				<tr>
					<td class="text-center">作物栽培要求：</td>
					<td><input type="text" name="Crop_period" class="form-control" /></td>
				</tr>
				<tr>
					<td class="text-center">作物栽培时间：</td>
					<td><input type="text" name="Crop_date" class="form-control" /></td>
				</tr>
				<tr>
					<td colspan="2"><button class="btn bcm tcw w3"
							style="margin-left: 80px;">添加</button></td>
				</tr>
				
				
			</table>
		</div>
	</form>

	<script type="text/javascript"
		src="<%=request.getContextPath()%>/assets/js/jquery.min.js"></script>
	<script type="text/javascript"
		src="<%=request.getContextPath()%>/assets/js/jquery.validate.min.js"></script>
	<script type="text/javascript"
		src="<%=request.getContextPath()%>/assets/js/messages_zh.min.js"></script>
	<script type="text/javascript"
		src="<%=request.getContextPath()%>/assets/js/form-ajax.js"></script>
</body>
</html>